...
const Navbar = () => {
  ...
  // Set active tab based on scroll position and detect scroll for navbar appearance
  useEffect(() => {
    const handleScroll = () => {
      const scrollPosition = window.scrollY;
      
      // Add shadow and background to navbar after scrolling down
      ...
      sections.forEach((section) => {
        ...
        if (scrollPositionWithOffset >= sectionTop && scrollPositionWithOffset < sectionTop + sectionHeight) {
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-50 flex justify-center p-6">
      <nav className={cn(
        "py-4 px-6 rounded-full backdrop-blur-md transition-all duration-300",
        scrolled ? "bg-white/80 shadow-lg" : "bg-white/40"
      )}>
        <div className="container mx-auto flex justify-between items-center max-w-6xl">
          ...
          <div className="hidden md:flex items-center gap-3 bg-white/50 border border-travel-ocean/20 backdrop-blur-lg py-1 px-1 rounded-full shadow-md">
            {navItems.map((item) => {
      ...
      <div id="mobile-nav" className="hidden md:hidden absolute top-full left-0 right-0 mt-4 mx-6 bg-white/90 backdrop-blur-sm rounded-xl border border-travel-ocean/10 shadow-lg py-4 px-6">
